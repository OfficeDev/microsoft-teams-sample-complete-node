...
export class AdaptiveCardDailog extends TriggerActionDialog {
    ...
    {
        // if replyToId is not null and if defined key in adaptive card data is present in incoming activity value
        // to check if this request is from an adaptive card process adaptive card submit action values, it's a work around that will be cleaned up later
        if (session.message.replyToId && session.message.value)
        {
            let payload = session.message.value;
